/* 敦煌复苏计划 — scripture-sort.js · 对话完毕后再显示界面 */
function initScrip(ov, cb) {
  ...
  var hts=[148,160,128,150,132,120], order=[], sel=-1, solved=false;
  ...
  function render() {
    ...
    order.forEach(function(orig,pos) {
      ...
      el.src=isSel?onSrc[orig]:offSrc[orig];
      ...
      el.addEventListener('click',function() {
        ...
        else{var t=order[sel];order[sel]=order[pos];order[pos]=t;sel=-1;render();if(order.every(function(v,i){return v===Answers.scripOrder[i];})) winS();}
      });
      row.appendChild(el);
    });
  }
  ...
  function winS() {
    solved=true; document.getElementById('s-status').textContent='✓ 顺序正确！'; document.getElementById('s-skip').style.display='none';
    var imgs=document.getElementById('s-row').querySelectorAll('img');
    imgs.forEach(function(el,i){ el.src=fixSrc[order[i]]||onSrc[order[i]]; el.style.border='2px solid var(--gold)'; el.style.cursor='default'; });
