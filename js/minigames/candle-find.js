/* 敦煌复苏计划 — candle-find.js · 小佛像烛光 · img标签 */
function initCandle(ov, cb) {
  ...
  ov.innerHTML = '<div style="display:flex;flex-direction:column;align-items:center;gap:10px;padding:12px;width:100%">' +
    ...
    '<div id="c-stage" style="position:relative;width:280px;height:280px;background:radial-gradient(ellipse at 50% 50%,rgba(20,12,5,.4),rgba(0,0,0,.95));border-radius:8px;overflow:hidden;border:1px solid rgba(200,150,60,.08)"></div>' +
  ...
  if (!hasClue) { Dialogue.play(Dialogues.statue_no).then(function() { cb(false); }); return; }
  Dialogue.play(Dialogues.statue_ok).then(function() { startC(); });
  ...
  function startC() {
    ...
    if (cur) cur.remove();
    document.getElementById('c-stage').querySelectorAll('.candle-light,.statue-img').forEach(function(e) { e.remove(); });
  ...
  function cycle() {
    if (done) return;
    if (cur) cur.remove();
    var stg = document.getElementById('c-stage'); if (!stg) return;
    ...
    sTm = setTimeout(function() { if (done) return; el.style.opacity = '0'; el.style.transform = 'scale(.5)'; document.getElementById('c-hint').textContent = '火光消失了……再找找？'; hTm = setTimeout(function() { if (!done) cycle(); }, 900); }, Config.candle.visible);
  }
  ...
  function winC() {
    ...
    var stg = document.getElementById('c-stage');
    var img = document.createElement('img'); img.className = 'statue-img';
    img.src = 'assets/小佛像/小佛像.png';
    img.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:contain;z-index:3;animation:fade-in .8s;';
    stg.appendChild(img);
    setTimeout(function() { Dialogue.play(Dialogues.statue_win).then(function() { cb(true); }); }, 700);
  }

  document.getElementById('c-retry').addEventListener('click', function() { startC(); });
  document.getElementById('c-skip').addEventListener('click', function() { done = true; clearTimeout(sTm); clearTimeout(hTm); Dialogue.play(Dialogues.statue_lose).then(function() { cb(false); }); });
}
