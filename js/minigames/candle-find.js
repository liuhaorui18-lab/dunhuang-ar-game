...
function initCandle(ov, cb) {
  ...
  function showUI() {
    ...
    document.getElementById('c-skip').addEventListener('click', function() { done=true;clearTimeout(sTm);clearTimeout(hTm);Dialogue.play(Dialogues.statue_lose).then(function(){ov.classList.remove('active');cb(false);}); });
  }
  ...
  function winC() {
    ...
    document.getElementById('c-hint').textContent='小佛像找到了！';document.getElementById('c-skip').style.display='none';
    ...
    stg.appendChild(img);
    setTimeout(function(){Dialogue.play(Dialogues.statue_win).then(function(){ov.classList.remove('active');cb(true);});},700);
  }
}
